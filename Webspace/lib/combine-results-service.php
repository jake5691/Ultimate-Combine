...
function uc_results_normalize_row(array $row): array {
...
function uc_results_context(PDO $pdo, int $teamId, int $combineId): ?array {
  $stmt = $pdo->prepare(
  ...
  $categoryWeights = uc_results_category_weights($pdo, $combineId);
  $results = uc_results_values($pdo, $teamId, $combineId);
  ...
  return [
    "combine" => uc_results_normalize_row($combine),
    "players" => $players,
...
function uc_results_players(PDO $pdo, int $teamId, int $combineId): array {
  ...
  return array_map("uc_results_normalize_row", $stmt->fetchAll());
}

function uc_results_disciplines(PDO $pdo, int $teamId, int $combineId): array {
  $stmt = $pdo->prepare(
  ...
  foreach ($stmt->fetchAll() as $row) {
    $row = uc_results_normalize_row($row);
...
function uc_results_category_weights(PDO $pdo, int $combineId): array {
  $stmt = $pdo->prepare(
...
function uc_results_values(PDO $pdo, int $teamId, int $combineId): array {
  $stmt = $pdo->prepare(
...
function uc_results_meta(array $context, int $teamId): array {
  return [
    "team_id" => $teamId,
